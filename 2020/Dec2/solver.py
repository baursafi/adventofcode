...
and at most 3 times. In the above example, 2 passwords are valid. 
...
din = [[w.strip().replace(':','').split('-') for w in x.split(' ')] for x in din]

a1 = sum([(eval(x[0][0])<=Counter(x[-1][0])[x[1][0]]) and (Counter(x[-1][0])[x[1][0]] <= eval(x[0][1])) for x in din])
print('Number of valid passwords is: ',a1)
...
a2 = sum([(x[1][0] == x[-1][0][eval(x[0][0])-1]) + (x[1][0] == x[-1][0][eval(x[0][1])-1]) == 1 for x in din])
