...
those drinks that come with a little umbrella, you are interrupted 
...
Then, the accumulator is increased from 0 to 1 (acc +1) and jmp +4 sets the next instruction 
to the other acc +1 near the bottom. 
...
instructions = [line.split(' ')[0] for line in inpt]
values = [eval(line.split(' ')[1]) for line in inpt]

df = pd.DataFrame(zip(instructions, values), columns=['instructions','values'])
df['count'] = 0
...
while True:
    ...
    count = df.loc[index, 'count']
    if count == 2:
        break
    instruction = df.loc[index,'instructions']
    if instruction == 'acc':
        accumulator += df.loc[index, 'values']
        index += 1
...
instruction below the last instruction in the file. 
With this change, after the program terminates, the accumulator contains 
...
found = False
for idx in tqdm(df[df.instructions.isin(['nop','jmp'])].index):
    df_copy = df.copy()
    if found == True:
        break
    if  df_copy.loc[idx, 'instructions'] == 'jmp':
        df_copy.loc[idx, 'instructions'] = 'nop'
    else:
        df_copy.loc[idx, 'instructions'] = 'jpm'
    count = 0
    index = 0
    accumulator = 0
    while True:
        if index >= len(df_copy):
            print(f"Instruction {df_copy.loc[idx, 'instructions']} in line {idx} must be changed")
            print('Accumulator = ', accumulator)
            found = True
            break
        df_copy.loc[index, 'count'] += 1
        count = df_copy.loc[index, 'count']
        if count == 2:
            break
        instruction = df_copy.loc[index,'instructions']
        if instruction == 'acc':
            accumulator += df_copy.loc[index, 'values']
            index += 1
        elif instruction == 'jmp':
            index += df_copy.loc[index,'values']
        elif instruction == 'nop':
            index += 1
