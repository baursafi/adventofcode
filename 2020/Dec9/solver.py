...
to your computer through the clever use of several paperclips. 
...
each number you receive should be the sum of any two of the 
25 immediately previous numbers. 
...
in a random order. To be valid, the next number must be the sum 
...
The first step of attacking the weakness in the XMAS data is to find t
he first number in the list (after the preamble) which is not the sum 
...
inpt = [eval(number) for number in inpt]

inpt = np.array(inpt)

for i, number in enumerate(inpt[26:]):
    preambule = inpt[i:i+26]
    variants = []
    for j, number1 in enumerate(preambule):
        for number2 in preambule:
            if number1 == number2:
                print('Found the match for: ',number,' = ', number1, '+', number2)
                time.sleep(.01)
            else:
                variants.append(number1 + number2)
    if number not in variants:
        print('The number that does not fit the rule after preaumbule is:', number)
        break

"""
--- Part Two ---
The final step in breaking the XMAS encryption relies on 
the invalid number you just found: you must find a 
contiguous set of at least two numbers in your list which 
...
through 40 produces the invalid number from step 1, 127. 
(Of course, the contiguous set of numbers in your actual 
...
What is the encryption weakness in your XMAS-encrypted 
list of numbers?
"""

start = 0
end = 1
# Let's run it as an expandable range between start at 0 and end at 1
# and every time the sum or the range is lower than the number
# we move the "end" one position to the right
# until the moment when the sum exceeds the number
# then we move "start" to the right and "end" to the left:
while True:
        i = sum(inpt[start:end])
        if i > number:
            start += 1
            end -= 1
        elif i < number:
            end += 1
        weakness = min(inpt[start:end]) + max(inpt[start:end])
        print("Weaknes for number:", number, "equals: ", weakness)
        break
